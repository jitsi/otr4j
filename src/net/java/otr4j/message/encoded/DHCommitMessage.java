...
public final class DHCommitMessage extends EncodedMessageBase {
	...
	public String toString(){
		...
		// Message type (BYTE)
		byte[] messageType = EncodedMessageUtils.serializeByte(this.getMessageType());
		len += messageType.length;
	
		// Encrypted gx (DATA)
		byte[] serializedGxEncrypted = EncodedMessageUtils.serializeData(this.gxEncrypted);
		len += serializedGxEncrypted.length;
	
		// Hashed gx (DATA)
		byte[] serializedGxHash = EncodedMessageUtils.serializeData(this.gxHash);
	...
	public DHCommitMessage(int protocolVersion, byte[] r,
			DHPublicKey gxKey, byte[] gxHash) throws InvalidKeyException, NoSuchAlgorithmException, NoSuchPaddingException, InvalidAlgorithmParameterException, IllegalBlockSizeException, BadPaddingException{
		this();
